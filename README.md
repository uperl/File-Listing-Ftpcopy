...
drop in replacement for the `parse_dir` function from [File::Listing](http://search.cpan.org/perldoc?File::Listing)
...
provided by [File::Listing](http://search.cpan.org/perldoc?File::Listing).  `ftpparse` is written in C, and so may or may
...
Where this module may come in handy over [File::Listing](http://search.cpan.org/perldoc?File::Listing) is that it understands
the output from a different subset of systems.  For my purposes, `ftpparse`
understands VMS listings, on the other hand, [File::Listing](http://search.cpan.org/perldoc?File::Listing) understands
...
or [File::Listing](http://search.cpan.org/perldoc?File::Listing), or if you do not know ahead of time, you can try each 
...
with [File::Listing](http://search.cpan.org/perldoc?File::Listing).
...
so this module is not, and does not pretend to be 100% compatible with [File::Listing](http://search.cpan.org/perldoc?File::Listing).
...
know what the remote server's time zone is.  [File::Listing](http://search.cpan.org/perldoc?File::Listing) assumes the listing is
...
hoops to support the [File::Listing](http://search.cpan.org/perldoc?File::Listing) interface.  To avoid these hoops use the `ftpparse`
interface instead.
...
- [File::Listing](http://search.cpan.org/perldoc?File::Listing)
